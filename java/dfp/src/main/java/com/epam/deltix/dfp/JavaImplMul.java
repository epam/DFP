...
public class JavaImplMul {
    ...
    public static long bid64_mul(final long x, final long y, final int rnd_mode /* = BID_ROUNDING_TO_NEAREST */) {
        ...
        final long valid_y;
        {
            ...
            if ((y & SPECIAL_ENCODING_MASK64) != SPECIAL_ENCODING_MASK64) {
                ...
                if ((y & INFINITY_MASK64) == INFINITY_MASK64) {
                    ...
                    if (UnsignedLong.compare(y & 0x0003ffffffffffffL, 1000000000000000L) >= 0)
        ...
        if (bin_expon_product < UPPER_EXPON_LIMIT + 2 * BINARY_EXPONENT_BIAS) {
            ...
            //__tight_bin_range_128(bp, P, bin_expon_product); // tighten exponent range
            {
                ...
                if (bp < 63) {
                    M <<= bp + 1;
                    if (UnsignedLong.compare(__P_w0, M) >= 0)
                        bp++;
                } else if (bp > 64) {
                    M <<= bp + 1 - 64;
                    if ((UnsignedLong.compare(__P_w1, M) > 0) || (__P_w1 == M && __P_w0 != 0))
                        bp++;
                } else if (__P_w1 != 0)
                    bp++;
            }
            ...
            round_up = 0;
            if (UnsignedInteger.compare(final_exponent, 3 * 256) >= 0) {
                if (final_exponent < 0) {
                    // underflow
                    ...
                    if (extra_digits > 17) {
                        //__mul_128x128_full(Q_high, Q_low, P, bid_reciprocals10_128[16]);
                        {
                            ...
                            //__add_128_128(QM, ALBH, AHBL); // add 128-bit value to 128-bit assume no carry-out
                            {
                                ...
                                if (UnsignedLong.compare(Q128_w0, __B128_w0) < 0)
                                    Q128_w1++;
                                _QM_w1 = Q128_w1;
                                _QM_w0 = Q128_w0;
                            }
                            ...
                            //__add_128_64((_Qh), AHBH, QM2_w1); // add 64-bit value to 128-bit
                            {
                                ...
                                if (UnsignedLong.compare(Q_high_w0, __B64) < 0)
                                    __R64H++;
                                Q_high_w1 = __R64H;
                            }
                        ...
                        extra_digits -= 16;
                        if (remainder_h != 0 || (UnsignedLong.compare(Q_low_w1, bid_reciprocals10_128_flat[(16 << 1) + 1]) > 0
                            || (Q_low_w1 == bid_reciprocals10_128_flat[(16 << 1) + 1]
                            && UnsignedLong.compare(Q_low_w0, bid_reciprocals10_128_flat[(16 << 1) + 0]) >= 0))) {
                            round_up = 1;
            ...
            if (extra_digits > 0) {
                ...
                //__add_128_64(P, P, bid_round_const_table[rmode][extra_digits]); // add 64-bit value to 128-bit
                {
                    ...
                    if (UnsignedLong.compare(P_w0, __B64) < 0)
                        __R64H++;
                    P_w1 = __R64H;
                }
    ...
    static boolean __unsigned_compare_gt_128(final long A_w0, final long A_w1, final long B_w0, final long B_w1) {
        return (UnsignedLong.compare(A_w1, B_w1) > 0) ||
            ((A_w1 == B_w1) && (UnsignedLong.compare(A_w0, B_w0) > 0));
    }
}
